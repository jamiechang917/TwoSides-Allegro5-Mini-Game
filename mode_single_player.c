...
static void win_scene(Map);
static void bonus_scene(ObjectQueue*, Object*, Map);
static void next_round_scene(int, Map, Object);
static void place_random_object(ObjectQueue*, Map);

void MODE_SINGLE_PLAYER_init() {
	//cursor
	init_cursor(&cursor);
	...
	init_character(&player, CHARACTER_01, "Player" , map.start_location[0], map.start_location[1] +56);
...
int MODE_SINGLE_PLAYER_run() {
	int round = 1;
	bool running = true;
	printf("Running (MODE_SINGLE_PLAYER)...\n");
	
	next_round_scene(round, map, player);
	round++;
	for (int i = 0; i < 3; i++) {
		place_random_object(&game_objects, map);
	}
	...
	while (running) {
		...
		if (event.type == ALLEGRO_EVENT_TIMER) { // refresh every 1/FPS seconds.
			...
			if (event.timer.source == timer_fps) { // regular timer
				...
				if (player.state == CHARACTER_GOAL || player.state == CHARACTER_DEAD) {
					stop_all_timer();
					reset_all_object(game_objects, map);
					if (player.character_score >= WIN_SCORE) {
						win_scene(map);
						running = false;
						return MODE_HOME;
					}
					if (round % 5 == 0) {
						bonus_scene(&game_objects, &cursor, map);
					}
...
void MODE_SINGLE_PLAYER_destroy() {
	//stop timer
	stop_all_timer();
	//destroy timer
	destroy_all_timer();
...
void bonus_scene(ObjectQueue* object_queue, Object* cursor, Map map) {
	al_draw_bitmap(map.background, 0, 0, NULL);
	ALLEGRO_FONT* font = al_load_font("./resources/font/pirulen.ttf", 60, NULL);
	char text[20] = "";
	snprintf(text, 20, "Bonus");
	al_draw_text(font, al_map_rgb(255, 195, 60), SCREENWIDTH / 2, SCREENHEIGHT / 2, ALLEGRO_ALIGN_CENTRE, text);
	al_flip_display();
	al_rest(3);
	al_destroy_font(font);
...
void next_round_scene(int round, Map map, Object character) { // enter next round
	al_draw_bitmap(map.background, 0, 0, NULL);
	ALLEGRO_FONT* font = al_load_font("./resources/font/pirulen.ttf", 60, NULL);
	...
	snprintf(text, 20, "Player  %d/%.0f", character.character_score, WIN_SCORE);
...
void reset_all_object(ObjectQueue object_queue, Map map) {
	...
	while (object != NULL) {
		switch (object->object_type) {
			case OBJECT_CURSOR:
				object->type = CURSOR_NORMAL_MODE;
				object->img_index = 0;
				break;
...
void place_random_object(ObjectQueue* object_queue, Map map) {
	int random_object_type = randi(OBJECT_PLATFORM_WOOD, OBJECT_BOMB);
	Object virtual_cursor;
	init_cursor(&virtual_cursor);
	virtual_cursor.x = randf(map.start_location[0] + 30, SCREENWIDTH);
	virtual_cursor.y = randf(0, SCREENHEIGHT);
	virtual_cursor.cursor_place_object_type = random_object_type;
	place_object(object_queue, virtual_cursor);
}